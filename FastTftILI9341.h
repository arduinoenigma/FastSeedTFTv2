/*
  ...
  now an arduino sketch originally written using the Seeed V2 library
  can be quickly converted to use this faster library
  since the code is the same for both the Adafruit and Seed screens
  a define switch was added to allow the code to work on either screen
  
  #define TFTSeed
  #define TFTAdafruit
  
  modified fillRectangle for pixel exact results with Seeed library
  
  to use in your .INO sketch:
  
											//1) uncomment one of the lines below
											//   this tells the library which screen to control
  #define TFTSeed
  //#define TFTAdafruit

  #include <SPI.h>
  #include <FastTftILI9341.h>  				//2) add this line to use the PDQ_ILI9341 library
											//   delete the include for the Seed Tft library

  PDQ_ILI9341 Tft;  						//3) modify this line for the Tft object to use the PDQ_ILI9341 library 
  
											//the rest of the program is any unmodified program for the Seeed TFT Screen.
  
  void setup()
  {
	//TFT initialization routine:
	Tft.TFTinit();  						// unchanged: init TFT library
  }
  
  void loop()
  {
	//start using library
	Tft.fillRectangle(0, 0, 240, 320, RED);	// this remains unchanged	  
  }
...
#if defined TFTSeed
#define	ILI9341_CS_PIN		5			// <= /CS pin (chip-select, LOW to get attention of ILI9341, HIGH and it ignores SPI bus)
#define	ILI9341_DC_PIN		6
#endif

#if defined TFTAdafruit
...
#error Oops!  The first line in your sketch must be either #define TFTSeed or #define TFTAdafruit
#endif

#include <FastTftFastPin.h>
...
class PDQ_ILI9341 : public PDQ_GFX<PDQ_ILI9341>
{
 public:
	...
	PDQ_ILI9341();
	static void inline TFTinit(void);
	...
	// required driver primitive methods (all except setPixel can call generic version in PDQ_GFX with "_" postfix).
	...
	static void fillRectangle(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color);
...
void PDQ_ILI9341::setPixel(int16_t x, int16_t y, uint16_t color)
{
...
void PDQ_ILI9341::fillRectangle(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	// increment w and h for compatibility with seeed library
	w++;
	h++;
	...
	//BUG: setAddrWindow_(x, y, x+w-1, _height);
